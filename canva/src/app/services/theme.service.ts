...
const presets: { [key: string]: Preset<any> } = {
  Aura,
  Material,
  Lara,
  Nora,
...
export class ThemeService {
  ...
  primaryColors = computed(() => {
    const preset = this.themeState()?.preset ?? 'Aura';
    ...
    const palettes = [{ name: 'noir', palette: {} as Preset<any> }];
  ...
  startViewTransition(state: ThemeState): void {
    const transition = (document as any).startViewTransition(async () => {
      await this.toggleDarkMode(state);
    });
